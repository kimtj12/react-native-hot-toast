...
const listeners: Array<
  [toasterId: string, listener: (state: ToasterState) => void]
> = [];

const defaultToasterState: ToasterState = {
  toasts: [],
  pausedAt: undefined,
  settings: { toastLimit: TOAST_LIMIT },
};
...
export const defaultTimeouts: { [key in ToastType]: number } = {
  blank: 4000,
  error: 4000,
  success: 2000,
  loading: Infinity,
...
export const useStore = (
  ...
  const typeOpts = (t: Toast) => toastOptions[t.type] ?? {};
  ...
  const mergedToasts = state.toasts.map((t) => ({
    ...
    ...t,
    removeDelay:
      t.removeDelay ?? typeOpts(t).removeDelay ?? toastOptions.removeDelay,
    duration:
      t.duration ??
      typeOpts(t).duration ??
      toastOptions.duration ??
      defaultTimeouts[t.type],
    style: [toastOptions.style, typeOpts(t).style, t.style].filter(Boolean),
    textStyle: [
      toastOptions.textStyle,
      typeOpts(t).textStyle,
      t.textStyle,
    ].filter(Boolean),
    iconStyle: [
      toastOptions.iconStyle,
      typeOpts(t).iconStyle,
      t.iconStyle,
    ].filter(Boolean),
  }));
